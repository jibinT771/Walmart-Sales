...
train_ds <-read_csv("sales_train_validation.csv")

View(train_ds)

prices_ds <- read_csv("sell_prices.csv")
...
calendar_ds <- fread("calendar.csv", 
                     stringsAsFactors = TRUE, 
                     drop = c("date", "weekday", "event_type_1", "event_type_2")) %>% 
  rename("date_id"=wm_yr_wk)

calendar_1 <- fread("calendar.csv", 
                    stringsAsFactors = TRUE, 
                    drop = c("date", "weekday", "event_type_1", "event_type_2")) 


train <- fread("sales_train_validation.csv", 
